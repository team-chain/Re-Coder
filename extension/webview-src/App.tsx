...
type ViewMode = "home" | "build" | "ship" | "operate" | "discord";
...
const Home: React.FC<HomeProps> = ({ isAiReady, isDockerReady, isOpsReady, discordConnected, onSelectMode }) => (
  <div style={{ padding: "14px 12px 10px", display: "flex", flexDirection: "column", gap: 9 }}>
    ...
    {/* Discord 연동 카드 — 항상 활성화 (연결 여부와 무관하게 설정 가능) */}
    <ActionCard
      icon={<Icon.Discord size={18} />}
      title="Discord 연동"
      description={discordConnected
        ? "봇 실행 중 — 모바일·Discord에서 /recoder 사용 가능"
        : "Discord 봇을 연결해 VSCode 밖에서도 ReCoder 사용"}
      accent="#5865f2"
      enabled={true}
...
const App: React.FC = () => {
  ...
  useMessage(
    useCallback((msg) => {
      ...
      // Discord 봇 상태 변경 → 홈 카드 업데이트
      if (type === "discord.botStatus") {
        setDiscordConnected((payload as { running: boolean }).running);
      }
    }, [])
  );
  ...
  const subTitle = view === "build" ? "에러 분석"
