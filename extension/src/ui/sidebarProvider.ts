 ...
 *
 * 참고: extension/src/sidebar/SidebarProvider.ts 는 v6.4 골격의 간결한 별도 구현입니다.
 * 본 파일은 Workbench 연동 / Git 폴링 / 다중 타이머 등 부가 기능을 포함한 풍부한 구현입니다.
 */
...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    private async _handleMessage(msg: { type: string; [key: string]: any }): Promise<void> {
        switch (msg.type) {
            ...
            case 'paste_error_log':
                // 기존 3-Mode 탭 UI 호환 — 붙여넣은 로그를 그대로 webview 로 반환
    ...
    private async _sendInitialState(): Promise<void> {
        try {
            ...
            // 통합형 initial_state 메시지 (3-Mode 탭 UI 호환)
            this.sendMessage({ type: 'initial_state', status, cost, ready });
            // 개별 메시지도 함께 전송 (개별 위젯 호환)
            if (ready) {
    ...
    private _startStatusPolling(): void {
        this._statusPollTimer = setInterval(async () => {
            try {
                ...
                this.sendMessage({ type: 'status_update', data: status, status, cost });
