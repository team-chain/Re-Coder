 ...
 *     - 'wb.tab'                         { tab: 'command'|'error'|'github'|'deploy'|'discord' }
 *     - 'wb.discord.loadConfig'          저장된 Discord 설정 요청
 *     - 'wb.discord.saveConfig'          Discord 설정 저장 { guildId, guildName, ... }
 *     - 'wb.discord.resolveInvite'       초대 링크 → 서버 정보 조회 { url }
 *     - 'wb.discord.openInvite'          OAuth2 봇 초대 URL 열기 { guildId? }
 *   extension → webview:
 ...
 *     - 'wb.discord.configLoaded'        { guildId, guildName, ..., connected }
 *     - 'wb.discord.inviteResolved'      { ok, guildId?, guildName?, guildIcon?, memberCount?, error? }
 *     - 'wb.discord.botStatus'           { guildId, guildName }
 */
...
export class WorkbenchPanel {
    ...
    static createOrShow(
        ...
        WorkbenchPanel._current = new WorkbenchPanel(panel, extensionUri, apiClient, coreManager, polling, context);
        return WorkbenchPanel._current;
    }
    ...
    private async _handleMessage(msg: { type: string; payload?: any }): Promise<void> {
        switch (msg.type) {
            ...
            // ── Discord 연동 ──────────────────────────────────────────────────
            case 'wb.discord.loadConfig':
                await this._discordLoadConfig();
                break;
            case 'wb.discord.saveConfig':
                await this._discordSaveConfig(msg.payload ?? null);
                break;
            case 'wb.discord.resolveInvite': {
                const { url } = (msg.payload ?? {}) as { url?: string };
                await this._discordResolveInvite(url ?? '');
                break;
            }
            case 'wb.discord.openInvite': {
                const { guildId } = (msg.payload ?? {}) as { guildId?: string };
                const inviteUrl = this._makeBotInviteUrl(guildId);
                await vscode.env.openExternal(vscode.Uri.parse(inviteUrl));
                break;
            }
    ...
    private static readonly DISCORD_CONFIG_KEY = 'recoder.discord.config';

    /** Bot OAuth2 초대 URL 생성. guildId 있으면 서버 선택이 미리 지정된다. */
    private _makeBotInviteUrl(guildId?: string): string {
        const clientId = vscode.workspace.getConfiguration('recoder').get<string>('discordClientId', '');
        const perms = '277025638400'; // Send Messages + Embed Links + Manage Webhooks + Read History
        let url = `https://discord.com/api/oauth2/authorize?client_id=${clientId}&permissions=${perms}&scope=bot%20applications.commands`;
        if (guildId) {
    ...
    /** 저장된 Discord 설정을 웹뷰로 전송한다. */
    private async _discordLoadConfig(): Promise<void> {
        const raw = this._context?.globalState.get<Record<string, string>>(WorkbenchPanel.DISCORD_CONFIG_KEY);
        if (raw?.guildId) {
            this._panel.webview.postMessage({
                type: 'wb.discord.configLoaded',
                payload: { ...raw, connected: true },
            });
        } else {
            this._panel.webview.postMessage({ type: 'wb.discord.configLoaded', payload: { connected: false } });
        }
    }

    /** Discord 설정을 globalState에 저장한다. */
    private async _discordSaveConfig(cfg: Record<string, string> | null): Promise<void> {
        if (!this._context) { return; }
        try {
            if (!cfg || !cfg.guildId) {
                await this._context.globalState.update(WorkbenchPanel.DISCORD_CONFIG_KEY, undefined);
                this._panel.webview.postMessage({ type: 'wb.discord.botStatus', payload: {} });
                this.addActivity('info', 'Discord 연동 해제됨');
                return;
            }
            const existing = this._context.globalState.get<Record<string, string>>(WorkbenchPanel.DISCORD_CONFIG_KEY) ?? {};
            const updated = { ...existing, ...cfg };
            await this._context.globalState.update(WorkbenchPanel.DISCORD_CONFIG_KEY, updated);

            // 봇 서버 자동 등록
            void this._autoRegisterWithBot(updated);

            this._panel.webview.postMessage({
                type: 'wb.discord.botStatus',
                payload: { guildId: updated.guildId, guildName: updated.guildName ?? '' },
            });
            this.addActivity('ok', `Discord 연동 완료 — ${updated.guildName || updated.guildId}`);
        } catch (err) {
            this._panel.webview.postMessage({
                type: 'wb.discord.botStatus',
    ...
    /**
     * discord.gg/CODE 형태의 초대 링크에서 서버 정보를 조회한다.
     * Discord 공개 API를 Node.js https 모듈로 직접 호출 (CSP 우회).
     */
    private async _discordResolveInvite(inviteUrl: string): Promise<void> {
        ...
        if (!code || code.length < 2) {
            this._panel.webview.postMessage({
                type: 'wb.discord.inviteResolved',
                payload: { ok: false, error: '유효하지 않은 초대 링크 형식입니다. discord.gg/코드 형식으로 입력해주세요.' },
            });
            return;
        }
        try {
            ...
            if (data.code === 10006) {
                this._panel.webview.postMessage({ type: 'wb.discord.inviteResolved', payload: { ok: false, error: '존재하지 않거나 만료된 초대 링크입니다.' } });
                return;
            }
            if (!data.guild) {
                this._panel.webview.postMessage({ type: 'wb.discord.inviteResolved', payload: { ok: false, error: '서버 정보를 가져올 수 없습니다. 공개 초대 링크인지 확인해주세요.' } });
                return;
            }
            ...
            this._panel.webview.postMessage({
                ...
                payload: {
                    ...
                    guildIcon:   guild.icon ? String(guild.icon) : null,
            ...
            this._panel.webview.postMessage({
                type: 'wb.discord.inviteResolved',
                payload: { ok: false, error: `서버 조회 실패: ${err instanceof Error ? err.message : String(err)}` },
            });
        }
    }

    /** VSCode 설정의 botServerUrl로 봇 서버에 guild 정보를 자동 등록한다. */
    private async _autoRegisterWithBot(cfg: Record<string, string>): Promise<void> {
        ...
        try {
            ...
            const httpModule = require(url.protocol === 'https:' ? 'https' : 'http') as typeof import('https');
            ...
            this.addActivity('ok', 'Discord 봇 서버 자동 등록 완료');
        } catch (err) {
            ...
            const req = https.request(
                {
                    ...
                    method: 'GET',
                    headers: { 'User-Agent': 'ReCoder VSCode Extension (https://github.com/recoder)' },
                },
                (res) => {
                    let body = '';
                    res.on('data', (chunk: Buffer) => { body += chunk.toString(); });
                    res.on('end', () => {
                        try { resolve(JSON.parse(body)); } catch { reject(new Error('Discord API 응답 파싱 실패')); }
                    });
                }
            );
            req.setTimeout(8000, () => { req.destroy(); reject(new Error('Discord API 타임아웃')); });
            req.on('error', reject);
            req.end();
        });
    }
