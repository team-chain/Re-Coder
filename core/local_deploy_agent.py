...
def _is_port_free(port: int, host: str = "127.0.0.1") -> bool:
    """주어진 호스트 포트가 사용 가능한지(LISTEN 안 됨) 확인."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(0.3)
            return s.connect_ex((host, int(port))) != 0
    except Exception:
        return True  # 확인 실패 시 진행 시도


def find_free_host_port(preferred: int, max_offset: int = 50) -> int:
    """
    선호 포트가 사용 중이면 +1, +2 ... 로 비어있는 첫 포트를 반환.
    50 개 시도 후에도 못 찾으면 preferred 그대로 반환 (배포 단계에서 docker 가 에러 메시지를 줌).
    """
    p = int(preferred)
...
class LocalDeployAgent:
    ...
    def __init__(self):
        ...
        self._last_deployment_record: Optional[DeploymentRecord] = None  # S-9: 자동 롤백용
    ...
    def run_container(
        ...
        # 포트 매핑 구성 — 호스트 포트가 이미 LISTEN 중이면 빈 포트로 자동 시프트.
        # UX: 사용자가 다른 앱 켜놓고 배포해도 'port already in use' 로 죽지 않음.
        port_bindings = []
        for port_map in plan.ports:
            host_port_raw = port_map.get("host", 8000)
            container_port = port_map.get("container", 8000)
            try:
                preferred = int(host_port_raw)
            except (TypeError, ValueError):
                preferred = 8000
            actual = find_free_host_port(preferred)
            if actual != preferred:
                logger.warning(
                    f"[local_deploy] host port {preferred} busy → using {actual}"
                )
                # plan.ports 도 갱신해 status / health URL 이 옳은 포트를 가리키게 함
                port_map["host"] = actual
            port_bindings.append(f"{actual}:{container_port}")

        if not port_bindings:
            port_bindings.append(f"{find_free_host_port(8000)}:8000")
    ...
    def deploy(
        ...
        if not health_ok:
            ...
            # 이전 배포 기록이 있으면 자동 롤백 시도 (§S-9)
            if self._last_deployment_record and self._last_deployment_record.rollback_target:
                logs.append(f"[ROLLBACK] 이전 버전으로 자동 롤백 시도...")
                rollback_result = self.rollback(self._last_deployment_record, workspace_path)
                logs.extend(rollback_result.logs or [])
                if rollback_result.success:
                    logs.append(f"[ROLLBACK] 자동 롤백 성공")
                else:
                    logs.append(f"[ROLLBACK] 자동 롤백 실패: {rollback_result.error}")
        ...
        # 성공한 배포 기록 저장 (S-9: 다음 롤백 시 사용)
        self._last_deployment_record = record
    ...
    def rollback(self, record: DeploymentRecord, workspace_path: str) -> DeployResult:
        ...
        # 롤백 컨테이너 실행 — 원본 record의 ports/env/health_check_path 재사용
        plan = DeploymentPlan(
            ...
            ports=getattr(record, "ports", [{"host": 8000, "container": 8000}]),
            env=getattr(record, "env", []),
            health_check_path=record.health_check_path or "/health",
        )
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if data.get("status") != DeployStatus.DEPLOYED.value:
                            continue
                        # enum 필드 변환 후 DeploymentRecord 재생성
                        data["method"] = DeployMethod(data.get("method", "local_docker"))
                        data["status"] = DeployStatus(data.get("status", "deployed"))
                        valid_keys = {f for f in (
                            "deployment_id", "project_id", "method", "image",
                            "image_digest", "git_commit", "container_name",
                            "health_check_path", "deployed_at", "rollback_target", "status",
                        )}
                        last_record = DeploymentRecord(**{
                            k: v for k, v in data.items() if k in valid_keys
                        })
                    except Exception as inner_e:
                        logger.debug(f"[local_deploy] record parse error: {inner_e}")
                        continue
        except Exception as e:
    ...
    def rollback_latest(self, project_id: str, workspace_path: str) -> DeployResult:
        """
        project_id의 마지막 성공 배포로 자동 롤백. server.py 자동 롤백 호출용.
        ...
        if record is None:
            return DeployResult(success=False, error="롤백할 배포 기록 없음", logs=[])
        if not record.rollback_target:
            return DeployResult(success=False, error="이전 버전 이미지 정보 없음", logs=[])
...
__all__ = ["LocalDeployAgent", "DeployResult", "find_free_host_port", "get_local_deploy_agent"]
