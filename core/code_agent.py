...
_MAX_PROMPT_BYTES  = 6_000   # 파일 한 개당 프롬프트에 박을 최대 본문
_MAX_TOTAL_PROMPT  = 30_000  # 전체 프롬프트 상한 (토큰 quota 보호)
...
_SYSTEM_INSTRUCTION = """당신은 시니어 DevOps/소프트웨어 엔지니어입니다.
에러 텍스트와 관련 파일을 분석하여 반드시 실행 가능한 코드 수정안을 JSON 형식으로 제공해야 합니다.
관련 파일이 없거나 불완전해도 에러 텍스트에서 원인을 추론하여 구체적인 수정안을 반드시 제시하세요.
...
def _build_prompt(error_text: str, files: list[dict]) -> str:
    ...
    if files:
        ...
        file_section = "\n(관련 소스 파일을 찾지 못했습니다. 에러 텍스트만으로 수정안을 추정하세요.)\n"
...
{{
  ...
  "patches": [
    {{
      "file": "관련 파일 섹션에 보인 경로 그대로 (파일을 특정할 수 없으면 에러에서 추정한 경로 사용)",
      "unified_diff": "--- a/경로\\n+++ b/경로\\n@@ ... @@\\n- 기존줄\\n+ 수정줄"
    }}
  ]
}}
...
중요 규칙:
- 반드시 patches 배열에 최소 1개 이상의 수정안을 포함하세요
- 관련 파일이 있으면 해당 파일의 실제 경로와 구체적인 unified_diff 를 작성하세요
- 관련 파일이 없으면 에러 메시지에서 파일 경로를 추론하거나, 가장 관련 있어 보이는 파일명을 추정하세요
- unified_diff 는 실제로 적용 가능한 최소 변경만 포함하세요 (파일 전체 출력 금지)
- InstallError/ImportError 등 패키지 문제는 requirements.txt 또는 setup.py 를 수정하는 패치를 제안하세요
"""
