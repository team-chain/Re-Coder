...
# ── 결과 확인 ─────────────────────────────────────────────────────────
if [[ -f "$DIST_PATH/recoder-core" ]] || [[ -f "$DIST_PATH/recoder-core.exe" ]]; then
  log "빌드 완료 ✓  →  $DIST_PATH/recoder-core"
  ls -lh "$DIST_PATH"/recoder-core* 2>/dev/null || ls -lh "$DIST_PATH/"
else
  err "빌드 결과물을 찾을 수 없습니다 ($DIST_PATH/recoder-core)."
  exit 1
fi
